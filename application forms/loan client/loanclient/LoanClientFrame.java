...
public class LoanClientFrame extends JFrame {
	...
	public LoanClientFrame() {
		...
		btnQueue.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				LoanRequest request = new LoanRequest(ssn,amount,time,nextReplyId++);
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					gateway = new ClientGateway(frame);
	...
	public void addReply(RequestReply<LoanRequest, LoanReply> rr, LoanReply reply) {
		rr.setReply(reply);
